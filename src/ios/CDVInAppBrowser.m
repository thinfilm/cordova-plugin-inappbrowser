...
- (void)createViews
{
    ...
    self.toolbar.userInteractionEnabled = YES;
     self.toolbar.barTintColor = [self colorFromRGBA:_browserOptions.toolbarcolor];
    ...
    if (_browserOptions.navigationbuttoncolor != nil) { // Set button color if user sets it in options
        self.forwardButton.tintColor = [self colorFromRGBA:_browserOptions.navigationbuttoncolor];
    }
    ...
    if (_browserOptions.navigationbuttoncolor != nil) { // Set button color if user sets it in options
        self.backButton.tintColor = [self colorFromRGBA:_browserOptions.navigationbuttoncolor];
    }
    ...
    if (_browserOptions.toolbarcolor != nil) {
      self.view.backgroundColor = [self colorFromRGBA:_browserOptions.toolbarcolor];
    }
...
{
    id value = [_browserOptions valueForKey:propertyName];
    if (value) {
        return value;
    } else {
        NSLog(@"No value for this property");
        return nil;
    }
}
...
- (void)setCloseButtonTitle:(NSString*)title : (NSString*) colorString
{
    ...
    self.closeButton.tintColor = colorString != nil ? [self colorFromRGBA:colorString] : [UIColor colorWithRed:60.0 / 255.0 green:136.0 / 255.0 blue:230.0 / 255.0 alpha:1];
...
- (void)showToolBar:(BOOL)show : (NSString *) toolbarPosition
{
    ...
    if (show) {
        ...
        if ([toolbarPosition isEqualToString:kInAppBrowserToolbarBarPositionTop]) {
            ...
            webViewBounds.origin.y += (toolbarFrame.size.height + STATUSBAR_HEIGHT);
            [self setWebViewFrame:webViewBounds];
        } else {
...
- (void) rePositionViews {
    if ([_browserOptions.toolbarposition isEqualToString:kInAppBrowserToolbarBarPositionTop]) {
        ...
        CGFloat webViewOriginY = TOOLBAR_HEIGHT + STATUSBAR_HEIGHT;
        CGFloat webViewHeight = self.webView.frame.size.height - ([self isiPhoneXAndHigher] ? 0 : STATUSBAR_HEIGHT);
        [self.webView setFrame:CGRectMake(self.webView.frame.origin.x, webViewOriginY, self.webView.frame.size.width, webViewHeight)];
        [self.toolbar setFrame:CGRectMake(self.toolbar.frame.origin.x, [self getStatusBarOffset], self.toolbar.frame.size.width, self.toolbar.frame.size.height)];
    }
}

- (BOOL)isiPhoneXAndHigher {
